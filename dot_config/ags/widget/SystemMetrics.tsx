...
function getNetSpeed(): { up: string; down: string } {
  ...
  return { up: formatBytes(txSpeed), down: formatBytes(rxSpeed) }
}
...
export function SystemMetrics() {
  const metrics = createExternal(pollMetrics(), (set) => {
    const fast = setInterval(() => set(pollMetrics()), 2000)
